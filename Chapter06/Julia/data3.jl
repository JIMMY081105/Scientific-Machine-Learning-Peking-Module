...
# one hidden tanh neuron WITH an output weight W1 and output bias b2, so the
# model is no longer stuck inside (-1,1):  NN(x) = W1 * tanh(W2*x + b1) + b2
#   W2 = input weight, b1 = input bias, W1 = output weight (amplitude),
#   b2 = output bias (vertical shift)
